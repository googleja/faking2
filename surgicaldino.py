...
class decode_head_linear(torch.nn.Module):
    ...
    def __init__(self, input_transform="resize_concat", image_shape=(224, 224), in_index=(0, 1, 2, 3), upsample=4,
                 min_depth=0.001, max_depth=150, classify=True,
                 n_bins=256, bins_strategy='UD', norm_strategy='linear', in_channels=[768, 768, 768, 768],
                 channels=6144, align_corners=False, scale_up=False, ):
        super().__init__()
    ...
    def forward(self, inputs, **kwargs):
        ...
        output = torch.nn.functional.interpolate(output, size=self.image_shape, mode="bilinear",
                                                 align_corners=self.align_corners)
        # print(output.shape)
        return output
...
class Dinov2ForDepth(torch.nn.Module):
    def __init__(self, backbone_size="base", image_shape=(224, 224), decode_type='linear4'):
        ...
        self.n = self.intermediate_layers[
            self.backbone_size] if decode_type == 'linear4' else 1  # self.intermediate_layers[self.backbone_size][-1]
    ...
    def save_parameters(self, filename: str) -> None:
        ...
        if isinstance(self.decode_head, torch.nn.DataParallel) or isinstance(self.decode_head,
    ...
    def forward(self, pixel_values, depth_gt=None):
        # use frozen features
        feature = self.dinov2.get_intermediate_layers(pixel_values, n=[2, 5, 8, 11], reshape=True,
        ...
        if depth_gt is not None:
            loss = self.sig_loss(pred.squeeze(), depth_gt.squeeze()) + self.gradient_loss(pred.squeeze(),
                                                                                          depth_gt.squeeze())
...
class SurgicalDINO(nn.Module):
    ...
    def forward(self, pixel_values, depth_gt=None):
        feature = self.dinov2.get_intermediate_layers(pixel_values, n=self.n, reshape=True, return_class_token=True,
                                                      norm=False)
        pred = self.decode_head(feature)
...
if __name__ == "__main__":
    ...
    surgicaldino = SurgicalDINO(backbone_size="base", r=4, lora_layer=None, image_shape=(224, 224),
                                decode_type='linear4').to(device=device)
